...
import {
  ...
  export class StatisticsController {
    ...
    async generateReport(
      ...
      try {
        const { fileName, filePath } = await this.statisticsService.generateReport(generateReportDto);
    ...
    async downloadReport(@Query('fileName') fileName: string, @Res({ passthrough: true }) res: Response) {
      try {
